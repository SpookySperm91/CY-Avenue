...
{
    private readonly DatabaseConnection dbConnector;
    private readonly ImageViewer imageViewer;

    public ClientSearch(DatabaseConnection dbConnector)
    {
    ...
    {
        return SearchClients("prcClientSearchByName", null, fullName, gender, "Member", out message);
    }
    ...
    public List<Client?> SearchWalkInByFullName(string fullName, out string message)
    {
        return SearchClients("prcClientSearchByName", null, fullName, null, "Walk-in", out message);
    }
    ...
    public List<Client> SearchAll(String type)
    {
        ...
        try
        {
            using (var connection = dbConnector.mysqlConnection)
            {
                connection.Open();
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            else
                            {
                                ...
                                client.SetProfilePictureImage(imageViewer.ConvertByteArrayToImage(client.ProfilePictureByte));
                            }
    ...
    private List<Client?> SearchClients(string storedProcedure, int? membershipId, string? fullName, string? gender,
    ...
    // private void MapClient(Client client, )
    // {
    //     client.FullName = reader["full_name"].ToString();
    //     client.MembershipId = Convert.ToInt32(reader["membership_id"]);
    //     client.MembershipTypeId = Convert.ToInt32(reader["membership_type_id"]);
